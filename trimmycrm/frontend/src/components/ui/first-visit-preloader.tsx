...
const INTRO_STORAGE_KEY = "trimmycrm:first-visit-intro:v1";
...
export const firstVisitPreloaderScript = `
  (() => {
    ...
    try {
      ...
      document.documentElement.dataset.trimmyIntro = seen ? "seen" : "show";
      if (!seen) window.sessionStorage.setItem(key, "1");
    } catch {
      document.documentElement.dataset.trimmyIntro = "show";
    }
  })();
`;
...
export function FirstVisitPreloader() {
  ...
  useEffect(() => {
    ...
    const fallback = window.setTimeout(finish, reducedMotion ? 800 : 3_100);
  ...
  return (
    ...
    >
      <div className="first-visit-preloader__brand" aria-hidden="true">
        <BrandMark compact />
      </div>
      <svg
        className="first-visit-preloader__signature"
        viewBox="0 0 638 200"
        fill="none"
        stroke="currentColor"
        strokeWidth="14.8883"
      ...
      >
        <path
          className="first-visit-preloader__stroke first-visit-preloader__stroke--lead"
          pathLength={1}
        ...
        />
        <path
          className="first-visit-preloader__stroke first-visit-preloader__stroke--word"
          pathLength={1}
      ...
      <span className="first-visit-preloader__accent" aria-hidden="true" />
    </div>
  );
}
